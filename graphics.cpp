...
void Graphics::HierarchicalUpdate2(double dt) {
	...
	localTransform *= glm::translate(glm::mat4(1.f), glm::vec3(0.f, cos(-dt / 2) * 2, sin(-dt / 2) * 2));
	modelStack.push(localTransform);			// store starship-sun coordinate
	
	localTransform *= glm::rotate(glm::mat4(1.f), (float)-((dt - 3.14159) / 2), glm::vec3(1.0f, 0.0f, 0.0f));
	localTransform *= glm::scale(glm::vec3(scale[0], scale[1], scale[2]));
